...
# 🚗 Vehicle Health & Service Tracker – Interactive & Safety-Focused
...
if [[ "$tire_wear" -lt 50 && "$charge_status" -gt 50 && "$engine_temp" -lt 100 && "$suspension_wear" -lt 50 && "$brake_wear" -lt 50 ]]; then
    echo -e "${GREEN}✅ Your vehicle is in good condition for the entered distance. Drive Safe!${NC}" >> "$LOG_FILE"
else
